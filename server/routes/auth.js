...
const loginLimiter = rateLimit({
  ...
  skip: (req) => {
    ...
    return IS_DEV && ["127.0.0.1", "::1", "::ffff:127.0.0.1"].includes(ip);
  },
...
router.put("/profile", authMiddleware, async (req, res) => {
  try {
    ...
    if (!store.update)
      return res.status(501).json({ error: "Modification non supportée" });
    ...
    if (newPassword && newPassword.length > 0) {
      if (!currentPassword) {
        return res
          .status(400)
          .json({ error: "Mot de passe actuel requis pour le changer" });
      }
      ...
      if (!pwCheck.valid) {
        return res
          .status(400)
          .json({ error: `Nouveau mot de passe : ${pwCheck.error}` });
      }
    ...
    if (fullName !== undefined && fullName !== null)
      updates.fullName = sanitizeString(String(fullName), 255);
